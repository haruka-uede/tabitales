...
4. **Sentence length**: EN sentences that stack multiple subordinate clauses or em-dash asides read as dense, hard-to-parse blocks when translated literally into a single JA sentence. Split these into two or more short JA sentences (one idea per sentence) rather than chaining clauses together with commas/て-form/が. Favor more, shorter paragraphs over fewer, longer ones — this applies throughout the body, not just the intro paragraph. This is a standing style rule for all JA articles on this site, not a one-off preference.
5. **Drop the "book itself" / translator-credit bullet**: the EN "Practical travel notes" list ends with a bullet naming the standard English translation (translator name, publisher — e.g. "the standard English translation, titled *Black Rain*, is by John Bester, published by Kodansha International"). This exists only to help an English-reading visitor find a copy in their own language; a Japanese reader is reading the original, so the bullet has no equivalent purpose. Omit it entirely from the JA body rather than translating it — don't replace it with anything.
6. At the end of your response, list what you trimmed or reframed for tone, so the human reviewer can sanity-check those specific calls rather than re-reading the whole diff line by line.
7. Do not commit, push, or merge anything yourself. Leave the new file as an uncommitted change so it goes through review below first — see why in Review.
